(function($) {
	...
	function xs_custom_function(argument) {
		...
		if (w_height.height() >= 750 && w_height.width() <= 991 && w_height.width() >= 1400) {
			xs_welcome.css('height', '400'+'px');
		} else {
			xs_welcome.css('height', '550'+'px');
		}
	...
	if ($('#xs-maps').length > 0) {
		...
		function init() {
			...
			var mapOptions = {
				...
				center: new google.maps.LatLng(9.087193, 7.474129),
			...
			var marker = new google.maps.Marker({
				position: new google.maps.LatLng(9.087193, 7.474129),
				map: map,
				title: 'PRSF',
